    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            materials.forEach(function (material) {
                $("#materialList").append('<li class="list-group-item">' + material.name +
                    '<button class="btn-danger btn-sm ml-2" onclick="deleteMaterial(\'' + material.id + '\')">Hapus</button>' +
                    '<button class="btn-warning btn-sm ml-2" onclick="populateMaterialForm(\'' + material.id + '\')">Ubah</button>' +
                    '</li>');
            });
        },
        error: function (error) {
...
function deleteMaterial(materialId) {
    $.ajax({
        type: "DELETE",
        url: "http://localhost:8080/api/materials/" + materialId,
        success: function (response) {
            setTimeout(() => {
...
function updateMaterial() {
    ...
    $.ajax({
        type: "PATCH",
        url: "http://localhost:8080/api/materials/" + updatedMaterialId,
        contentType: "application/json",
